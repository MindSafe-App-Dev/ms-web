...
export default function SmsPage() {
    ...
    const ensureTrialOnFirstUse = async () => {
        ...
        const trial = await consumeTrialAccess(user.$id, deviceId, PREMIUM_TRIAL_FEATURE_IDS.sms);
        if (!trial.allowed) {
            showWarning('Trial Limit Reached', trial.message);
            showPremium('Premium Feature', 'SMS trial limit reached. Upgrade to continue.', () => router.push(buildPremiumRoute(deviceId)));
            return false;
        }
